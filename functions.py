# -*- coding: utf-8 -*-
"""
Created on Fri Jan 26 09:22:26 2024
...
def load_system(config, direc, name):
    # config_direc = 'C:/Users/felix/Desktop/System Analysis/Current/'
    with open(direc + name + '.json', 'r') as system_file:
        system = system_file.read()
    system = json.loads(system)
    system['Config'] = config
    return system

def write_system(system, direc, file_name):
    # file_name = str(system['Config']['Flight Time [s]']) + 's_' 
    # file_name += str(system['Config']['Fuel Pump']) + 'fuel_pump_' 
    # file_name += str(system['Config']['LOx Pump']) + 'ox_pump_' 
    # file_name += str(system['Config']['Number Press Reducers']) 
    # file_name += 'press_reducers' + str(system['Config']['Number Press Tanks']) 
    # file_name += 'press_tanks.json'
    
    # file_name = 'incorrect_oxodizer_tank_mass_and_thickness.json'
    # ROF = system['Propulsion']['Thrust Chamber']['Oxidizer Fuel Ratio']['Value']
    # fuel = system['Config']['Fuel'] .replace("'", "")
    
    # file_name = f'{ROF=}_{fuel=}.json'
    
    with open(direc + file_name, 'w') as system_file:
...
def calc_effective_exhaust_velocity(system, comb, indicator): # !!! ,p_ex=1e5):
    if indicator == 'max':
        p_cc = system['Propulsion']['Thrust Chamber']['Chamber Pressure']['Value']
...
# def calc_isentropic_exhaust_velocity(system, indicator, p_ex=1e5, eta_comb=1):
#     if indicator == 'max':
#         p_cc = system['Propulsion']['Thrust Chamber']['Chamber Pressure']['Value']
#     elif indicator == 'min':
#         p_cc = system['Propulsion']['Thrust Chamber']['Chamber Pressure Min']['Value']
...
#     T, M, gamma = obtain_combustion_properties(system, comb, p_cc)
#     v_ex = np.sqrt((2*gamma*R_ideal)/(gamma-1)*(T)/(M)*(1-(p_ex/p_cc)**((gamma-1)/gamma)))
#     system['Propulsion']['Thrust Chamber'][f'Isp_{indicator}'] = {'Type': 'Output', 'Value': v_ex*eta_comb}
#     return system

    
# def ziolkowsky(system, mode):
#     if mode == 'design':
...
#         m_dry = get_system_mass(system, ["Fuel", "Oxidizer", "Pressurant"])
        
#         if 'Mass Change' in system['Propulsion']['Pressurant'].keys():
#             m_press_init = system['Propulsion']['Pressurant']['Mass']['Value']
#             m_press_end = m_press_init - system['Propulsion']['Pressurant']['Mass Change']['Value']
#             m_prop = (m_dry + m_press_end)*np.exp(deltav/v_ex) - (m_dry + m_press_init)
...
#         # print(v_ex, deltav, m_dry, m_prop)
#         return system
